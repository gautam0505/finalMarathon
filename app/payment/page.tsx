...
export default function PaymentPage() {
  ...
  useEffect(() => {
    const generateUpiLink = async () => {
      ...
      const response = await fetch(
        `/api/generate-upi-link?amount=${finalAmount}&transactionId=${newTransactionId}`
      );
  ...
  const handleBackToDetails = () => {
    router.push("/register"); // Redirect to the register page (or the appropriate page/step)
  };
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      // Allow only alphanumeric characters and limit the length to 12
                      if (/^[a-zA-Z0-9]{0,12}$/.test(inputValue)) {
                        setUserTransactionId(inputValue);
                      }
                    }}
                    required
                    maxLength={12} // Ensures the input doesn't exceed 12 characters
                  />
                  {/* Optional error message if the input is invalid */}
                  {userTransactionId &&
                    !/^[a-zA-Z0-9]{12}$/.test(userTransactionId) && (
